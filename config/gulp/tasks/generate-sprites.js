...
const generateSpritesTask = () => {
    ...
    spriteNames.forEach(name => {
        ...
        const retina = gulp.src(`${src}/${spriteSrc}/${name}-2x/*.png`).pipe(
            spritesmith({
        ...
        const regular = gulp.src(`${src}/${spriteSrc}/${name}-1x/*.png`).pipe(
            spritesmith({
    ...
    stream.on('end', () => {
        log(
            colors.yellow(
                `Combined sprite @1x and @2x images have been generated in ${colors.green(
                    spriteDest
                )}.`
            )
        );
        log(
            colors.yellow(
                `Sass map information files have been generated in ${colors.green(
                    sassDest
                )}.`
            )
        );
                ...
                log(
                    colors.yellow(
                        `Created ${colors.green(
                            sassFileName
                        )}. This file should be imported into your "_sprites.scss" mixin.`
                    )
                );
        ...
        log(
            colors.red(
                'See https://spothero.com/uniform/ace/docs/tasks-sprites/ for more details.'
            )
        );
    });
