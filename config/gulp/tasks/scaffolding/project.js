...
const confirmScaffoldProjectTask = () => {
    return gulp
        ...
        .on('end', () => {
            log(
                colors.red(
                    "Make sure to run `npm start -- installPeerDeps` before development if you didn't install `peerDependencies` yet."
                )
            );
    ...
    inquirer
        .prompt([
            {
                ...
                type: 'list',
                message:
                    'Scaffold default ACE project files? (you can do this later by calling `npm start -- scaffoldProject`)',
                choices: [
                    'Standard',
        ...
        .then(answers => {
            ...
            if (type === 'Standard' || type === 'SpotHero') {
                ...
                sequence('confirmScaffoldProject', cb);
            } else {
                cb();
            }
        });
