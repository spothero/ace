...
const getEnvironment = () => {
    ...
    if (npmEnvironment === 'sandbox' && isNil(process.env.SANDBOX_NAME)) {
        throw new PluginError(
            'deploy',
            'You must specify a sandbox name for sandbox deployments. See README.md for more information.'
        );
    }
...
const invalidateCloudFront = cb => {
    ...
    const distroIds =
        global.SETTINGS_CONFIG.deploy[npmEnvironment].cloudFrontDistributionId;
    const params = {
        InvalidationBatch: {
        ...
        gulp.task(taskName, taskCb => {
            return new Promise(() => {
                cloudfront.createInvalidation(distro, (err, data) => {
                    if (err) {
                        throw new PluginError('invalidate', err, {
                            showStack: true,
                        });
                    } else {
...
const uploadToS3 = () => {
    ...
    if (!['sandbox', 'staging', 'production'].includes(npmEnvironment)) {
        throw new PluginError(
            'uploadToS3',
            'Environment must be sandbox, staging, or production.'
        );
    }

    return gulp
        .src([`${projectPath(global.SETTINGS_CONFIG.dist.path)}/**`])
        .pipe(
            s3(
                {
...
const deployTask = cb => {
    ...
    if (
        isEmpty(settings.path) ||
        isEmpty(settings[npmEnvironment].bucket) ||
        isEmpty(settings[npmEnvironment].cloudFrontDistributionId)
    ) {
        throw new PluginError(
            'deploy',
            'You must fill in all the applicable settings for the "deploy" object in the settings file. See https://spothero.com/uniform/ace/docs/tasks-deploying/ for details.'
        );
    }

    sequence('upload', 'invalidate', cb);
};
